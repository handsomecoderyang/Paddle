...
inline CUDAStream getCurrentCUDAStream(DeviceIndex device_index = -1) {
  if (device_index == -1) {
    // CustomDevice: get current device
    device_index = phi::DeviceManager::GetDevice("metax_gpu");
  }

  // CustomDevice: get stream from context
  auto& pool = paddle::platform::DeviceContextPool::Instance();
  auto* dev_ctx = pool.Get(phi::CustomPlace("metax_gpu", device_index));
  auto* custom_ctx = static_cast<phi::CustomContext*>(dev_ctx);

  return CUDAStream(reinterpret_cast<cudaStream_t>(custom_ctx->stream()));
}
